...
class FlvTagImpl {
    constructor(offset, data, global_metadata) {
        this.details = {};
        this.analysis = {};
        this.offset = offset;
    ...
    _parse_audio_data(meta) {
        if (!this.data.length)
            return;
        const flags = this.data[0];
        const sound_format = flags >> 4;
        this.details["Format"] = AUDIO_FORMATS[sound_format] || `Unknown (${sound_format})`;
    ...
    _parse_video_data() {
        ...
        this.details["Frame Type"] = VIDEO_FRAME_TYPES[frame_type] || `Unknown (${frame_type})`;
        this.details["Codec ID"] = VIDEO_CODECS[codec_id] || `Unknown (${codec_id})`;
    ...
    get_type_name() {
        return TAG_TYPES[this.tag_type] || `Unknown (${this.tag_type})`;
    }
    get_display_info() {
        const info = {
